...
def insert(user_id, month, give_rmb, cost_rmb, cost_month):
...
def select_by_uid(user_id):
    conn = sqlite3.connect(DB_URL)
    execute = conn.cursor()
    execute.execute("select * from p2p_tb where user_id = ? order by code asc;",
                    (user_id,))
    ...
    for res_one in res_list:
        ...
        e.give_rmb = res_one[3]
        e.cost_rmb = res_one[4]
...
def select_month_range(uid_list):
    ...
    :return: 月份中文数组列表
    """
    sql = """select min(dt.code),max(dt.code) from p2p_tb cdt 
...
def select_cost_by_uid(user_id, file_month):
    ...
    execute.execute("""select ppt.* from (
        select  cost_month, sum(cost_rmb) as cost_rmb  from p2p_tb 
