...
from dao import basic_user, date_dao, c7_dao
from util import print_excel_util, excel_reader_util


def load_c7():
    file_dict = excel_conf.get_c7_file_conf()
    for file_name in file_dict:
        load_one(file_name)


def load_one(file):
    xlsx_file = openpyxl.load_workbook(filename="./source/c/" + file,
                                       data_only=True)  # 打开文件
    file_conf = excel_conf.get_c7_file_conf()[file]

    sheet_name = excel_conf.get_c7_file_conf()[file].read_sheet_name
...
def load_sheet(default_sheet, file_conf):
    ...
    for x in range(row_max - title_size):
        ...
        if user is None:
            ...
            continue
        c7_dao.insert(user.code, file_conf.file_month, rmb)
...
def print_line(sheet, user_list, column_num, point_seq):
    ...
    date_list = c7_dao.select_month_range(uids)
    if len(date_list) == 0:
...
def print_header(sheet, row_len, cur_row, point_seq, date_list):
    title = str(point_seq) + "） C类推广员7%推广奖励：  "
...
def print_person(sheet, user_one, row_len, cur_row, date_list):
    ...
    ab_data_list = c7_dao.select_by_uid(user_id=user_one.code)
