...
def load_sheet(default_sheet, file_conf, flag):
    ...
    for i in range(100):
        ...
        if val is not None and str(val).replace('\n', '').replace('\r', '').replace('\t', '') \
                .replace(' ', '') == file_conf.read_bound_row:
            rmb_column = i + 1
            break
    if rmb_column == 0:
        print("加载ab文件未找到" + file_conf.read_bound_header_title + "列.文件名：" + file_conf.file_name)
        return
    last_region_str = None
    last_part_str = None
    for x in range(row_max - title_size):

        region_str = default_sheet.cell(row=x + title_size + 1, column=1).value
        if region_str is not None and region_str != "":
            last_region_str = region_str

        part_str = default_sheet.cell(row=x + title_size + 1, column=2).value
        if part_str is not None and part_str != "":
            last_part_str = part_str
        part = last_part_str

        true_name = default_sheet.cell(row=x + title_size + 1, column=3).value
        work = default_sheet.cell(row=x + title_size + 1, column=4).value
        if work == '业务员':
            work = '业务'
